...
public static class EfFilterCriteriaConverter
{
    ...
    private static Expression BuildFilterExpression(ParameterExpression parameter, Filter filter)
    {
        ...
        var constant = Expression.Constant(parsedValue, isNullable ? targetType : underlyingType);
    ...
    private static IQueryable<T> ApplyOrdering<T>(IQueryable<T> query, Order order)
    {
        ...
        var methodName = order.OrderType.Type == OrderTypes.ASC ? "OrderBy" : "OrderByDescending";
